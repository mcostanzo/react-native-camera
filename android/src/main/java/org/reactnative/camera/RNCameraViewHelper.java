...
public class RNCameraViewHelper {
  ...
  public static void emitFacesDetectedEvent(final ViewGroup view, final WritableArray data, final byte[] compressedImage, final int width, final int height, final int orientation) {

    final ReactContext reactContext = (ReactContext) view.getContext();
    reactContext.runOnNativeModulesQueueThread(new Runnable() {
      @Override
      public void run() {
        FacesDetectedEvent event = FacesDetectedEvent.obtain(view.getId(), data, compressedImage, width, height, orientation);
        reactContext.getNativeModule(UIManagerModule.class).getEventDispatcher().dispatchEvent(event);
      }
     });
  }
